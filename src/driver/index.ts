...
// Resolve relative to this module so optional native deps load from the host app.
// This loads better-sqlite3 / node:sqlite and never runs in the browser (where a
// driver such as @monlite/wasm's wasmDriver is passed explicitly). The static
// `node:module` import is correct on ALL Node versions (ESM + CJS); for a BROWSER
// bundle, alias/stub `node:module` (see the demo's vite.config — `node:module`).
