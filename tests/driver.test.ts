...
describe("drivers", () => {
  ...
  it.skipIf(!HAS_NODE_SQLITE)("node:sqlite backend works (Node >= 22.5)", async () => {
    ...
    const g = await c.groupBy({ by: ["n"], _count: true, orderBy: { n: "asc" } });
  ...
  it.skipIf(!HAS_NODE_SQLITE)("both backends produce identical results", async () => {
    ...
    for (const d of [a, b]) await d.collection("u").createMany({ data: seed });
